...
| --- | --- | --- | --- | --- |
| **G-15** | El contrato no expone una consulta de reconciliación para una carga de tarjeta por `topup.id` (`GET /topup/:id` o equivalente). Un replay puede devolver `processing`/`requires_action` sin `client_secret`; desde el front no se puede acreditar ni liberar esa intención con certeza. | Topup con tarjeta y 3DS/reintentos B-06. | Conserva el journal y bloquea una nueva carga; muestra copy honesta para revisar el saldo luego. No inventa polling ni rota la clave. | **Pendiente ORDEN 2C:** App Backend debe ratificar un endpoint/shape de reconciliación y sus estados terminales. Anotado en auditoría 2026-08-02. |
