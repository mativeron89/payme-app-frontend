...
| --- | --- | --- | --- | --- |
| **G-38** | **El contrato no publica CON QUÉ TARJETA se garantizó una mesa.** El backend SÍ lo guarda —`auth_source_payment_method_id`, `auth_card_brand`, `auth_card_funding`, `auth_charge_payment_method_id`— y lo usa internamente (`storedMesaCardSnapshot` en `routes/mesas.js:131`, y el replay lo recupera en `:335`), pero **ninguna respuesta pública lo expone**: el 201 de `POST /mesas` manda `guarantee: {method, status, client_secret?, connected_account_id?}` (`:620-627`) y `GET /mesas/creations/:key` manda `guarantee: {method, authorized, connected_account_id?}` (`:796-801`). La omisión parece deliberada — el comentario de `:142-145` dice que la fila durable *"contiene IDs de Stripe, ownership, flags y snapshots… nunca deben filtrarse por un spread"*. | **Reconciliación de una apertura congelada tras un reload.** `loadCards()` autoselecciona la tarjeta DEFAULT, así que si la garantía se hizo con una guardada NO-default la pantalla la atribuía a la default — y con los botones deshabilitados, sin poder corregirlo. **No es una falla monetaria** (la fuente durable la conserva el backend y el replay la reusa) **pero sí una representación visual FALSA**. | **Fail-closed con la información existente, sin inventar campo (ORDEN 1-B):** ninguna tarjeta se autoselecciona mientras un intento exige reconciliación (`SIN_TARJETA_ELEGIDA`), la pantalla DICE que no puede mostrar cuál fue, y el reenvío exige elección explícita — porque si el diagnóstico da `not_found` la creación ocurre por primera vez y esa fuente **sí** respalda la garantía. El radio de SALDO sigue duro: `guarantee_method` está en `PAYLOAD_KEYS` y cambiarlo es otra intención económica. | **Pedido al dueño (App Backend):** exponer en `GET /mesas/creations/:key` un identificador NO sensible de la fuente durable — bastaría el `payment_method_id` interno (uuid, no el `pm_` de Stripe) o la dupla marca + últimos cuatro, que es lo que la UI necesita para decir la verdad. Con eso la tarjeta correcta se restaura y el estado "no sabemos" desaparece. Anotado 2026-08-07. |
| **G-37** | **La identidad económica del riel de PAGO no se puede alinear desde acá, y no es un olvido.** Desde la ORDEN 2-A el journal de `create_mesa` sella con el `payloadHash` del dueño (`PAYLOAD_KEYS.create_mesa`), que **deja la fuente de pago afuera a propósito** — incluirla rota la clave en un reload con tarjeta tipeada y abre una segunda mesa con un segundo hold (B-06). **`mesa_pay` tiene el mismo defecto**: el comentario del dueño en `utils/idempotency.js` lo dice con todas las letras (*"NO incluir: fuente de pago. Stripe.js puede materializar otro pm_ al reintentar una respuesta perdida"*). Pero el dueño mantiene **DOS tablas para ese riel — `mesa_pay` y `mesa_pay_legacy` — y desde el front no se puede saber cuál aplica a un request dado.** | Reintento de un PAGO tras respuesta ambigua + recarga, con tarjeta tipeada: `prepareMonetaryRequest` corta con `monetary_payload_ambiguous`. | **El riel de pago conserva el fingerprint del request ENTERO, que es MÁS estricto: traba de más, nunca de menos.** No se adivina cuál tabla aplica: un hash mal elegido produce el mismo síntoma que se viene a eliminar, y sobre el riel que mueve plata por cada pago. `economicKeysFor()` devuelve `null` para todo lo que no sea `create_mesa`, con test que lo fija. | **Pedido al dueño (App Backend):** (a) declarar cuál tabla aplica y cuándo, de forma consumible desde el front; y (b) publicar vectores canónicos de `mesa_pay` como los de `create-mesa-identity-vectors.json`. Con eso la alineación es mecánica y acreditable. Anotado 2026-08-07. |
