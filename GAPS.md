...
| --- | --- | --- | --- | --- |
| **G-24** | El build `/live/` acepta `?demo=1` y sustituye Stripe Elements por el PaymentMethod público de test `pm_card_visa`; también saltea la cámara OCR. Cualquier visitante puede activar el flag desde la URL. | Garantía y pago con tarjeta del artefacto que apunta al backend configurado; repo, deploy y credenciales externas no acreditan qué entorno está activo. | Se conserva por su uso histórico de demo. Solo es admisible en artefacto/host separado, acceso y datos aislados, y un entorno acreditado explícitamente como Stripe test. | **P1 pre-release/piloto:** el artefacto live debe compilar el modo demo inalcanzable; query/hash no deben cambiar UI ni requests y `pm_card_visa`/bypass OCR deben quedar ausentes o inaccesibles. Requiere coordinar cómo se conserva la demo; no se cambia unilateralmente en esta auditoría. |
