...
| --- | --- | --- | --- | --- |
| **G-40** | **El contrato no distingue, dentro de `my_bps`, qué parte de un ítem es mía RESERVADA y qué parte es mía PAGADA** (`contract-mirror/routes/mesas.js:1274-1284`: `my_bps` suma claims `locked` y `paid`). Con eso el front no puede saber si mi parte de un ítem a medio pagar ya está pagada. | «Soltar» un consumo (n80, dueño v2.100.0). Ofrecerlo sobre algo pagado prometería una acción que el dueño no hace (devuelve `released: []`). | **Se ofrece «Soltar» sólo cuando lo mío no puede estar pagado:** mesa `open` y `paid_amount_cents === 0`, en consumo, ítem no pagado entero y `locked_by_me`. Hoy, con los pagos apagados, eso cubre todos los casos reales. Con pagos vivos, después del primer pago de la mesa «Soltar» deja de ofrecerse. La etiqueta «Lo elegiste» sí se muestra, porque sigue siendo cierta. | **Abierto · pedido al dueño:** un campo aditivo por ítem, p. ej. `my_locked_bps` (o `my_paid_bps`), para ofrecer «Soltar» también en una mesa con pagos. Anotado 2026-09-19 (AF-25). |
