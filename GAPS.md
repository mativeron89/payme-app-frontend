...
La fuente local de este cierre es App Backend
`9c5a7b1409ccbac65cdb757c91c3daff5d100fac` (v2.90.0). El espejo de
`contract-mirror/` está pineado en `940cc49e` (v2.90.0, "commit A" del aviso
2.4.1) — 3 commits detrás del HEAD real del backend, los tres sin cambio de
contrato (`chore(mirror)`, `docs(changelog)`, `test(legal)`). App Frontend
está en `898f72b3` (v0.162.0). No hubo push, deploy ni consulta a producción;
leer un ref de git no acredita qué corre en vivo. Remedido 2026-09-17/18
(órdenes APP-GAPS-REMEASURE-AF-03-20260917 y APP-DOCS-PWA-AF-07-20260918).
...
| --- | --- | --- |
| ✅ ~~P0 Connect~~ **HECHO_LOCAL** | App Backend cerró la degradación monetaria: `cc5356c6164cd8fadc3088dedd627fe7728a2dbc` introdujo un gate tipado que nunca devuelve `null`, y `11af0a658e9e258e7d9d3dd2368f49c07005c8b4` lo conectó a garantía y pago. Las operaciones nuevas sin cuenta Connect apta fallan cerradas; los bindings históricos `NULL` quedan en cuarentena y no se reemiten. | Medido sobre App Backend `7c547456`. No acredita staging, producción ni Stripe real. D1-D, D1-E y los demás NO-GO no refutados permanecen independientes. |
| ✅ ~~P0 contractual (G-11)~~ **CERRADO DE VERDAD (2026-08-06, v2.47.0 `aa28e84`, espejado en R3-A)** | El primer cierre (`7e45db0`) fue REFUTADO —cinco huecos; el peor: la wallet nativa se adjuntaba a Stripe antes de validarla—. `aa28e84` lo cierra en la causa: **elegibilidad verificada ANTES de cualquier mutación remota**, y la promesa **converge** (`card_save_intents` + sweep: si el guardado falla por timeout, la tarjeta aparece en el tick siguiente en vez de perderse). Cero campos nuevos: el consumo de `0d9c475` era correcto y ahora está respaldado. Reauditado leyendo el código espejado, no el anuncio. | — |
| **P1 (D1-E)** | **Fase 1 construida, fase monetaria no.** `services/connectDisputeProcessor.js` (backend `9c5a7b14`) ingiere y reconcilia disputas de Stripe Connect contra `payment_attempts`/mesas, con proyección `closed_lost`/`closed_no_loss`/`manual_review` — pero su propio docstring dice explícitamente "no crea refunds, no devuelve application fees" y devuelve `monetaryAction: false`. Sigue sin existir el disparo automático del refund proporcional de la comisión de PayMe cuando el restaurante pierde una disputa. | App Backend, fase 2. Remedido 2026-09-17. |
| **P1 (D1-D)** | Los pagos tardíos quedan en revisión manual; faltan avisos siempre y el auto-refund ratificado para casos inequívocos de hasta MXN 2.000. Reverificado 2026-09-17 contra backend `9c5a7b14`: sin literal `200000`/umbral ni función `autoRefund` en `services/`/`routes/`. Sin cambio. | App Backend, sin ampliar la decisión ratificada. |
| **P1 privacidad** | `tables_count` YA se publica (`services/aggregateEmitter.js`); el mínimo de cohorte fue retirado por decisión de producto (min=1 para todos, acta 2026-08-16), así que esa mitad del gap quedó resuelta por política, no por código pendiente. **Sigue faltando la retracción**: el propio código dice "NO introduce semántica de retracción: eso es OLA 3B y exige acta coordinada". Remedido 2026-09-17. | App Backend + acta de producto para retracción. |
...
### Programa de corte del viernes (C1–C6) y aviso legal 2.4.x — consumidos, no documentados aquí

Entre `330a645c` (ancla vieja de este archivo) y `9c5a7b14` el backend publicó
el programa `FRIDAY-PUBLIC-NO-PAYMENTS-AUTONOMOUS-01` (alta pública bajo
`PUBLIC_SIGNUP_ENABLED` conviviendo con invitaciones, mesa sin garantía bajo
`MODO_MONETARIO='disabled'`, `GET /mesas/mine` para el histórico propio) y los
avisos de privacidad 2.4.0/2.4.1. **Este front ya lo consumió** (`feat(alta)`
`85bdab5`, `feat(corte)` `8d161ac`, `feat(legal)` `2758270`, espejos `6d702b8`
y `80e7c27`). No es un gap de contrato — el front ya se movió —, es que este
archivo no tenía ninguna línea sobre eso. Anotado 2026-09-17 desde la
re-medición AF-03.

**Cabo suelto real: `GET /api/mesas/mine` (C4, v2.85.0) está espejado en
`contract-mirror/` pero ningún archivo de `src/` lo consume todavía.** Ver
G-39 abajo.
...
| --- | --- | --- | --- | --- |
| **G-41** | ✅ **CERRADO POR EL EMISOR (App Backend v2.125.0, `bd1c976`, inventario `8e1c718`, espejado el 2026-09-23).** Cada grupo de `GET /api/account/stats/ingredients` trae `dish_count`: platos DISTINTOS del grupo, con la clave de `/stats/dishes`; la suma de los grupos es `distinct_dishes`. Antes (v2.115.0–v2.124.0) sólo traía `times` (visitas) y `amount_cents`. | «Qué comes · por ingrediente» (2d). El diseño de Mati dice «Pasta · 5 platos» en cada fila y el total de platos al centro del anillo. | **Con el dato:** el anillo reparte PLATOS, al centro «N platos distintos» (los mismos de «Platos»), y cada fila «N platos» y su monto, sin visitas ni porcentaje, como 2c y 2e. Al juntar una clave desconocida en «Otros» los platos se suman (cada plato cae en un solo grupo). **Sin el dato** (dueño anterior): la vista de siempre, con el dinero al centro y visitas, monto y porcentaje por fila. Todos los grupos lo traen o ninguno; una respuesta mezclada se rechaza. | **Cerrado 2026-09-23 (AF-ROADMAP-TARDE, n178).** 🔴 Hasta este cambio el decodificador exigía las tres claves exactas, así que con el dueño v2.125.0 servido la pestaña caía en su cartel de error. |
| **G-40** | ✅ **CERRADO POR EL EMISOR (App Backend v2.103.0, `343e57b`, espejado el 2026-09-19 en `bb1c46e`).** El dueño publica por ítem `my_paid_bps` (lo mío pagado) y `my_releasable_bps` (lo que `items/release` soltaría ahora, con la misma definición que la ruta). Antes, `my_bps` sumaba reservado y pagado sin distinguirlos. | «Soltar» un consumo (n80) en una mesa con pagos. | **Con el dato:** «Soltar» se ofrece si y sólo si `my_releasable_bps > 0`, también con pagos de otros (`partially_paid`), y la fila distingue lo pagado de lo elegido («Pagaste ½ · elegiste ½ más», «Ya lo pagaste»). **Sin el dato** (backend servido hoy, o un valor raro): la regla provisoria de cero pagos en la mesa. | **Cerrado 2026-09-19 (AF-29, n171).** Queda la regla vieja sólo como compatibilidad con el backend anterior. |
| **G-39** | **No es un gap de contrato: es consumo pendiente del front.** El backend publicó `GET /api/mesas/mine` (C4, v2.85.0) para el histórico propio del comensal ("Mis ítems" y el histórico del corte sin pagos), y el espejo del contrato ya lo tiene (`contract-mirror/routes/mesas.js`, `contract-mirror/README.md` lo cita: "C4 - A · App Backend v2.85.0"). | Historial (§1.10) y "Mis ítems": sin este endpoint, el comensal no tiene de dónde leer su histórico propio de mesas sin garantía/pagos del programa del corte. | **Ningún consumo visual todavía**: cero archivos de `src/` llaman a `mesas/mine` (verificado por grep sobre `898f72b3`). El contrato está espejado pero no cableado a ninguna pantalla. | **Pendiente de un tier propio del front** (probablemente T5, historial/Mis ítems). No exige nada nuevo de App Backend — el endpoint ya existe y está espejado. Anotado 2026-09-17/18 desde AF-03/AF-07. |
| **G-38** | Históricamente el contrato no publicaba con qué guardada se garantizó una mesa; el reload congelado no podía atribuir la fuente sin mentir. | Reconciliación de apertura con garantía. | El decoder exige `saved_payment_method_id` UUID interno o `null`; la pantalla restaura sólo si ese UUID sigue en `GET /payment-methods`, nunca compara ni expone `pm_`. Una referencia ausente/eliminada conserva `SIN_TARJETA_ELEGIDA`; una exacta queda bloqueada durante el replay. | ✅ **CERRADO owner-first en Backend v2.70.0 (`87a9a74`, publicado por `d1a22c9`) y consumidor v0.146.0.** El owner deriva el UUID exclusivamente de opener + fuente durable; tests rechazan `pm_` y restauraciones por default. |
| **G-37** | Históricamente el front no podía saber si `mesa_pay` usaba el keyset legacy o vigente y conservaba un fingerprint más grueso. | Reintento de pago tras respuesta ambigua o reload. | El owner publicó selector ejecutable por `payment_attempts.idempotency_hash_version`: ausente/v1 → `mesa_pay_legacy`; v2+ → `mesa_pay`. Nuevos journals front se sellan v2; los viejos conservan su `fpv`. Fuente y lock no cambian identidad; consumos, propina y destinatario sí. | ✅ **CERRADO owner-first en Backend v2.70.0 (`87a9a74`, publicado por `d1a22c9`) y consumidor v0.146.0.** Ocho vectores canónicos, hash absoluto y paridad contra el JS espejado quedan ejecutados en tests. |
| **G-36** | **No es un pedido de contrato: es deuda del riel de demo, anotada por orden del Bibliotecario (H-9 de la auditoría 2026-08-06).** Los vencimientos del seed son relativos a la PRIMERA carga (`iso(+12/26/29 min)`, `store.ts`) y el estado persiste en localStorage: a los ~30-45 min de sesión, todas las mesas abiertas del seed expiraron y la demo decae — le pasó al teléfono de Mati en plena noche de demos. | La demo larga: Inicio queda sin mesas, "Sumarme" desaparece (post-H-7) y el flujo sólo se recupera con **Más → Reiniciar la demo**. | "Reiniciar la demo" existe y cura todo, pero nada lo sugiere cuando el seed se pudrió. | ✅ **RESUELTO EN DOS TRAMOS — y sólo el segundo alcanza a los teléfonos que YA estaban rotos.** 🟢 **Tramo 1 (2026-08-06, orden 2-A.4) · entre sesiones, con seed NUEVO:** `relanzarSeedVencido()` corre AL HIDRATAR desde persistencia (nunca en caliente): las mesas con `seedRelanzable` —la parte viva de la demo— cuyo reloj quedó atrás y que el usuario NUNCA tocó vuelven a su estado sembrado con el vencimiento adelante, con la invitación atada al reloj de su mesa. Lo tocado (pago propio, canje, casillero o consumo reclamado) no se reescribe jamás; PA-1099 no lleva la marca (su historia ES estar cerrada); las mesas del usuario tampoco. La expiración real dentro de una sesión sigue intacta — se relanzan RELOJES entre sesiones, no se congelan. Acreditado con reloj controlado (`seedRelanzable.test.ts`: crear → +2 h → rehidratar → coherente) y mutante. 🟢 **Tramo 2 (2026-08-06, orden 1-C·B) · LEGACY MIGRADO:** un `localStorage` anterior a `67fc0de` no tiene la marca, y ése es justo el estado que ya estaba podrido en los dispositivos existentes. `migrarSeedLegacy()` se la pone **sólo a lo que se puede acreditar**: código en lista blanca (PA-1099 EXCLUIDA — su historia es estar cerrada), código único en el estado (los códigos nuevos salen del mismo rango y pueden colisionar), firma inmutable idéntica (total, modo, comensales, restaurante, `openedByUser`, `guarantee_method`), `paid_amount_cents` intacto, nadie la tocó (ahora **'guest' también cuenta**: es la misma persona), y la invitación sembrada todavía presente. La plantilla sale de una **tabla explícita**, nunca del estado persistido —que está sucio por definición— y un test la mantiene alineada con el seed. Todo corre en un `try/catch` propio: el de `loadPersisted` descarta el estado ENTERO. **Lo que no se acredita se CONSERVA**, y entonces Inicio ofrece la recuperación honesta ("Los datos de ejemplo de esta demo ya vencieron" + Reiniciar), sólo en mock. Acreditado con estado legacy real, reloj controlado y mutantes. |
| **G-35** | **No es un pedido de contrato: era deuda interna del router.** Una ruta desconocida escrita a mano (`#/saldo`, `#/zzz`) renderizaba Inicio pero dejaba el hash sucio en la barra de direcciones. Medido en auditoría 2026-08-06. | Higiene de URL: un link compartido o guardado conservaba un hash que no correspondía a nada. Cosmético. | `resolveHash` distingue una ruta reconocida del fallback a Inicio. Cuando degrada, `normalizeUnknownHash` reemplaza la entrada por `#/home`, sin dejar el hash inválido vivo en Atrás; si History API falla, la asignación de hash es el fallback explícito. | ✅ **RESUELTO LOCALMENTE en v0.145.1 (2026-08-25).** La clase queda cerrada por el parser: páginas desconocidas y parámetros mal codificados normalizan; la población completa de `PAGES`, hashes default, deep links y queries conocidas queda intacta. `useRoute` lo ejecuta tanto al montar como en cada `hashchange`. Rojo causal y 14 casos en `src/router.test.ts`. Sin push ni deploy. |
| **G-34** | ✅ **CERRADO POR EL EMISOR (App Backend v2.93.0, `7bb5b7b4`, espejado el 2026-09-18 en `fd5d301`).** `GET /api/mesas/open` publica `my_status` (`paid | pending | not_applicable`) y `my_paid_cents`, calculados **sólo para la cuenta del bearer**, según el handoff del dueño. Decisión de Mati del 2026-09-18, «Según lo que eligió cada uno» (sha256 `74443664…b4`). Desde `0.169.0`, con la mesa en curso: `paid` → «Ya pagaste, faltan otros», `pending` → «Te falta pagar», más «Pagaste $X» con lo propio. `not_applicable`, un valor ausente o uno desconocido mantienen la etiqueta genérica de la mesa. La decisión vive en `utils/labels.ts` (`estadoPersonalDeMesa`, `pagadoPropioCentavos`) y es la misma para la burbuja y la hoja. ⌁ **Original:** **`GET /mesas/open` no trae ningún campo por-participante**: ni `my_paid_cents` ni `my_status` — es dato de mesa entera (verificado en `contract-mirror/routes/mesas.js:651`, la proyección no toca `mesa_participants` más que para autorizar). | El badge de la burbuja de Inicio (§1.1). Quien **ya pagó su parte** en una mesa `partially_paid` no puede leer un badge personal ("Ya pagaste, faltan otros"): el front no tiene con qué distinguirlo. | **Etiqueta genérica y honesta que describe a la MESA**: `partially_paid` → **"Pago en curso"** (`utils/labels.ts`, fuente única para burbuja y hoja de "+N mesas"). Corrección de honestidad del spec 2026-08-05: la etiqueta vieja "Falta pagar" se leía como deuda propia. **No se personaliza sin el dato** — fingir "ya pagaste" sin campo sería inventar un estado. | **Pedido aditivo al dueño del contrato (App Backend), CONDICIONADO:** recién cuando `/mesas/open` (o el detalle) publique el estado de pago DEL PARTICIPANTE corresponde una etiqueta personal real ("Ya pagaste, faltan otros" / "Te falta pagar"). La condición está escrita también en el spec §1.1. Hasta entonces, genérica-y-honesta gana a personal-y-falsa. Anotado 2026-08-06 desde la auditoría. Reverificado 2026-09-17 contra `9c5a7b14`: `GET /mesas/open` sigue sin `my_status`/`my_paid_cents` por participante. Sin cambio. |
| **G-33** | ✅ **CERRADO POR ENDPOINT OWNER-ONLY.** El dueño expone `GET /api/account/movements/:id` y filtra en la misma query `pa.id = $1 AND pa.user_id = $2` (`contract-mirror/routes/account.js`): un pago ajeno y uno inexistente producen el mismo `404 movement_not_found`. La proyección devuelve únicamente los ítems de ese intento. En consumo publica importe/fracción realmente cobrados; en igualdad los mantiene `null` y publica por separado `declared_fraction_bps` dentro del conjunto natural cerrado `2500·3333·5000·6667·7500·10000`. Un registro histórico sin declaración sigue en `null`. | Historial (§1.10). Una mesa puede agrupar varios pagos propios porque pagar varias partes está ratificado. | El acordeón consulta **todos** los `payment_ids` agrupados; un decoder estricto valida forma, centavos, pares importe/fracción y la declaración separada antes de renderizar. La carga es lazy y cacheada; un error no fabrica detalle y el botón reintenta la red de verdad. En igualdad muestra la fracción declarada sin inventar precio por plato ni alterar el monto del casillero. | **Cerrado localmente en el consumidor (2026-08-25) contra mirror v2.68.0, 87/87.** No usa `GET /mesas/:code`, no depende de participación activa y no amplía autoridad. El endpoint sigue siendo dueño del backend y el mirror fija su forma. |
| **G-32** | 🔴 **El contrato no tiene invitaciones a grupo, y NO es un endpoint que falte: es otro modelo de producto.** Un grupo del contrato es **una etiqueta privada de UNA persona**, no una entidad compartida. `friend_groups` tiene `user_id` y `UNIQUE (user_id, name)` (`contract-mirror/db/schema.sql:683`); `GET /groups` filtra `WHERE g.user_id = $1` (`routes/groups.js:15`), así que **a quien agregás nunca se le avisa y el grupo no le aparece en su propio listado**; `POST /groups/:id/members` exige amistad aceptada e **inserta directo** con `201 {added:true}` (`:88`), sin dejar ningún estado pendiente que alguien pueda aceptar. Búsqueda de invitación de grupo en todo el espejo (`.js` y `.sql`, sin distinguir mayúsculas): **cero coincidencias**. | Solicitudes (§1.9). El spec pide adentro un **selector de pastilla Amigos / Grupos con su propio contador cada uno**, y filas *"Te invitó a {grupo}"* con **Aceptar** y **rechazar**, *"igual que el pedido de amistad, sin asimetría entre las dos listas"*. | **La pestaña sale con UNA lista —pedidos de amistad— y sin el selector.** No se construye la pastilla con el lado de Grupos vacío: un control cuyo segundo lado **no puede tener nada nunca** es exactamente la promesa vacía que el spec ya le negó al QR de Compartir (§1.7), a Cuentas Asociadas (§1.11) y a "Configuración" en `Más` — *una función real aparece cuando hay algo real detrás*. Tampoco se simula del lado del front: inventar una invitación que el emisor no emite es inventar contrato. | **Decisión de producto ANTES que pedido de contrato, y por eso no alcanza con un campo aditivo.** Que un grupo pase a ser compartido toca qué ES un grupo: tabla de invitaciones, estado de membresía, notificación al invitado y a quién le pertenece el grupo cuando lo integran varios. Requiere acta ratificada y después una orden coordinada App Backend↔App Frontend. **Mientras tanto el spec §1.9 tiene una mitad no implementable, y queda anotado ahí y acá.** Anotado 2026-08-05 desde §1.9. Reverificado 2026-09-17 contra `9c5a7b14`: `friend_groups`/`friend_group_members` siguen sin tabla de invitación pendiente. Sin cambio de contrato ni de schema. |
| **G-31** | ✅ **CERRADO POR EL EMISOR (App Backend v2.93.0, `96fd5575`, espejado el 2026-09-18 en `fd5d301`).** `GET /api/invitations` publica `restaurant_category` con el enum cerrado `italian | japanese | mexican | cafe | other`, según el handoff del dueño. La tarjeta de Avisos dibuja su ícono (`pasta`, `sushi`, `taco`, `coffee`) desde `0.169.0` (`utils/labels.ts` · `iconoDeCategoriaRestaurante`). `other`, un valor ausente o uno desconocido siguen en `store`, y el nombre nunca se usa para adivinar. ⌁ **Original:** **`GET /api/invitations` no dice de qué restaurante es la invitación, más allá del nombre.** La proyección es `id, mesa_id, invitation_type, status, expires_at, created_at, mesa_code, restaurant_name, inviter_*` (`contract-mirror/routes/invitations.js:22-36`): manda `r.name` y **ni `r.category` ni `r.id`**. Tampoco hay forma indirecta: `GET /restaurants/:id` es pública pero necesita el uuid, que la invitación no trae, y `GET /mesas/:code` exige ser participante — que es exactamente lo que todavía no sos cuando estás mirando la invitación. | Avisos (§1.8). El spec pide para la tarjeta de invitación un **"ícono de categoría del restaurante"**. | **Ícono genérico `store`, que no afirma ninguna cocina.** Antes había un **`sushi` hardcodeado**: eso no era un genérico, era decirle a la persona que el restaurante es japonés sin que nadie nos lo hubiera dicho. No se infiere del nombre —"Hanzo Sushi" es adivinar por subcadena y falla con cualquier nombre de fantasía— ni se pide `GET /restaurants/:id` sin uuid. | **Pedido al dueño del contrato (App Backend).** Campo aditivo: `restaurant_category` con el enum que ya existe en `restaurants.category` (`italian \| japanese \| mexican \| cafe \| other`), o el `restaurant_id` para resolverlo con el endpoint público que ya está. No cambia el shape. Anotado 2026-08-05 desde §1.8. Reverificado 2026-09-17 contra `9c5a7b14`: `routes/invitations.js` sigue sin `restaurant_category`. Sin cambio de contrato. |
| **G-30** | **El contrato NO expone quiénes están en una mesa, y es a propósito en todo salvo en un caso.** `mesa_participants` se INSERTA en tres lugares (`contract-mirror/routes/mesas.js:447`, `routes/invitations.js:102` y `:236`) y **no se SELECCIONA en ninguna respuesta**. `GET /mesas/:code` proyecta ítems, `division_slots`, `active_staff` y `my_role`, nada más: los slots ajenos van sin dueño —el comentario del propio contrato dice que "jamás expone de quién es el ajeno"— y `MesaItem` sólo trae `locked_by_me`. `GET /invitations` lista únicamente las **dirigidas al usuario actual** y **pendientes**, así que tampoco sirve para que el organizador vea a quién ya se le sumó. | Compartir (§1.7). El spec pide una pestaña **"Ya se sumaron"**: *"lista simple de quienes ya canjearon el link, mismo formato de fila que los contactos, sin botón de acción"*. Es una de las dos pestañas en burbuja de la pantalla; sin ella la pantalla no tiene el componente que el spec le pide. | **La pantalla no se implementa hasta resolverlo.** No se infiere de `division_slots` ni de los locks: eso listaría a quien **tomó algo**, que no es lo mismo que quien **se sumó**, y encima cruzaría la línea que el contrato cuida —los casilleros ajenos no tienen dueño visible—. Tampoco se arma con las invitaciones enviadas: quien entra por el link de WhatsApp nunca tuvo una fila de invitación in-app. | **Pedido al dueño del contrato (App Backend), y ojo que NO es sólo aditivo.** Listar participantes por nombre en una respuesta de mesa **es una decisión de privacidad**, no un campo más: hoy la mesa está construida para no decir quién es quién. La pregunta para el emisor es si el ORGANIZADOR —y sólo él, que ya es `my_role: 'opener'`— puede ver la lista de quienes canjearon, y con qué proyección (¿nombre de pila?, ¿`payme_id`?). Anotado 2026-08-05 desde §1.7. Reverificado 2026-09-17 contra `9c5a7b14`: `GET /mesas/:code` sigue sin lista de participantes. Sin cambio de contrato ni de schema. |
...
| **G-27** | ✅ **CERRADO POR EL EMISOR (App Backend v2.93.0, `96fd5575`, espejado el 2026-09-18 en `fd5d301`).** `GET /api/mesas/open` publica `participants_count` en cada mesa: el conteo de filas **activas** de `mesa_participants` (los que se sumaron, no `expected_participants`), según `docs/HANDOFF_CAMPOS_ADITIVOS_V2.93.0.md` del dueño (sha256 `c9a0dc8f…d4`). El front lo consume en `0.169.0` (`APP-MESAS-ADDITIVE-CONSUMER-AF-18`): «Mesa PA-2847 · 4 personas» en la burbuja y en la hoja de «+N mesas», con singular para 1. Sin el campo, o con un valor que no es entero ≥ 0, la línea va sin conteo como antes (`utils/labels.ts` · `personasEnMesa`). ⌁ **Original:** **`GET /api/mesas/open` no dice cuánta gente hay en la mesa.** La proyección devuelve `id, code, full_name, restaurant{name,category}, total_cents, paid_amount_cents, pct_paid, status, expires_at` (`contract-mirror/routes/mesas.js:597-607`): ni `expected_participants` —que existe en la tabla `mesas` y se usa en la creación (`:151`, `:406`)— ni un conteo real de participantes. | Inicio (§1.1). El spec pide textualmente la línea *"Mesa PA-2847 · 4 personas"* en la burbuja de la mesa abierta. | **La línea va sin el conteo: sólo "Mesa PA-2847".** No se infiere de `division_slots` ni de ningún otro lado — serían dos números distintos (los que el organizador declaró al dividir vs. los que efectivamente se sumaron) y el front no puede saber cuál quiso decir el spec. Un número inventado en la pantalla principal es peor que un dato de menos. | **Pedido al dueño del contrato (App Backend).** Campo aditivo en la proyección, sin cambiar el shape. **La decisión de fondo es del emisor:** si es `expected_participants` (los que se esperan) o el conteo de `mesa_participants` (los que están). Anotado 2026-08-05 desde §1.1. Reverificado 2026-09-17 contra `9c5a7b14`: la proyección de `GET /mesas/open` sigue sin `expected_participants` ni conteo de `mesa_participants`. Sin cambio. |
| **G-26** | ✅ **CERRADO owner-first.** App Backend publicó la señal por ítem en v2.49.0 (`415651c`); el contrato sigue vigente en el mirror v2.70.0 (`87a9a74`, inventario publicado por `d1a22c9`). `contract-mirror/services/ocrResponseContract.js:4-10,20-22,42-50,65-80` fija `confidence`, `low_confidence`, `warnings` y `total_detected_cents`. ⌁ **Original (2026-08-05, preservado):** **`OcrResponse` no trae ninguna señal por ítem de que el OCR no haya podido leerlo.** `POST /api/ocr` devuelve `{ items: [{name, price_cents, quantity, category?}], total_cents, mock }` (`contract-mirror/routes/ocr.js`, espejado en `src/api/types.ts:435`): no hay `confidence`, ni flag de no reconocido, ni marca de campo faltante. Un ítem que el proveedor leyó mal llega indistinguible de uno leído bien. | Ticket (§1.3). El spec pide una fila con borde punteado `--warning`, ícono de interrogación y placeholder *"¿Qué es esto?"* — el estado *desconocido* del sistema aplicado a una fila. | ✅ **Consumido fail-closed en frontend v0.78.0 (`fabcb2b3` + `3990158`) y vigente en v0.146.0.** El tipo conserva las señales (`src/api/types.ts:675-695`), el decoder rechaza formas y enums no contractuales sin inferir (`src/api/contractResponses.ts:81-139`) y `CreateMesaFlow` abre la revisión, diferencia la fila y muestra el aviso accesible sólo ante `low_confidence === true` (`src/screens/CreateMesaFlow.tsx:654-669,1498-1568`). Tests: `src/api/ffContracts.test.ts:43-81`, `src/screens/ocrScanView.test.ts:17-58` y `src/screens/ocrSurface.test.ts:9-30`. ⌁ **Original (2026-08-05, preservado):** **No se implementa.** El spec lo deja además pendiente de ver en pantalla. El front NO lo infiere de `price_cents === 0` ni de un nombre vacío: serían heurísticas inventadas del lado del consumidor sobre un dato que el emisor no calificó, y pintarían de "no reconocido" ítems que el OCR sí leyó. Lo que sí existe hoy es el contraste del total impreso contra la suma de las filas, que detecta el agregado pero no señala cuál fila lo causó. | ✅ **Pedido satisfecho por el owner y adoptado por el consumidor; no queda cambio contractual ni productivo abierto en G-26.** La señal es aditiva y opcional: el mock histórico no la inventa, Textract la aporta y el consumidor sólo actúa cuando el contrato la acredita. ⌁ **Pedido histórico (2026-08-05, preservado):** **Pedido al dueño del contrato (App Backend).** Alcanza con un campo aditivo y opcional por ítem — `confidence` numérico, o un booleano tipo `uncertain` — sin cambiar el shape existente. Anotado 2026-08-05 desde §1.3. |
| **G-25** | ✅ **CERRADO owner-first, y compatibilidad legacy RETIRADA (2026-09-18).** App Backend v2.71.0 (`064c51a`, inventario completo publicado por `5b1e1f2`) crea un recibo UUID opaco para **cada** POST y proyecta outgoing exclusivamente como `{id, requested_at}` (`contract-mirror/routes/friends.js`); la migración puebla salientes históricas. ⌁ **Original (2026-08-04, preservado):** **`GET /api/friends/requests?direction=outgoing` delata si una cuenta existe, y con quién.** `POST /friends` estaba construido para NO decirlo, pero el backend sólo insertaba fila cuando el destino existía y el GET saliente proyectaba `payme_id`, nombre y apellido. Presencia/ausencia + PII reabrían el oráculo. | Pantalla de amigos, sección "Enviadas". | ✅ **Consumido fail-closed en frontend v0.147.0, y desde v0.165.1 SÓLO acepta el DTO nuevo** — el shape viejo (`{requested:true}` sin id; outgoing con `user`) ahora es un error de contrato, no un caso a tolerar. `contractResponses.ts` exige `request_id` en el POST y rechaza cualquier saliente con `user`; incoming conserva persona sin cambios. `SocialScreen` cancela por receipt id y sólo retira tras el 200 exacto; error conserva/reconcilia. El mock crea/persiste un recibo por intento sin variar cardinalidad. Tests: `friendRequestContracts.test.ts` (con los dos mutantes del retiro), `friendRequestCancellation.test.ts`, `friendRequestsView.test.ts`, `mockFriends.test.ts` y `e2e/g25-recibos-opacos.spec.ts`. | ✅ **Pedido satisfecho por el owner, adoptado por el consumidor, y compatibilidad legacy retirada.** Compuerta E0 (`E0_G25_COMPUERTA_READONLY_20260918.md`, PASS) acreditó que el único deployment activo de App Backend desciende de `064c51a`/`5b1e1f2` (producción en `9c5a7b14`, 46/46 migraciones) y que ambos Frontends del release apuntan sólo a ese backend — la orden sucesora de 12 paths (`APP-G25-LEGACY-RETIRE-AF-10-20260918`) retiró la tolerancia. ⌁ **Pedido histórico preservado:** crear siempre una salida y proyectarla mediante identificador opaco sin revelar existencia/identidad. |
