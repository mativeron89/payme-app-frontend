...
| --- | --- | --- | --- | --- |
| **G-41** | **`GET /api/account/stats/ingredients` (v2.115.0) no trae cuántos PLATOS hay por grupo.** Cada grupo trae `times` (VISITAS con al menos un plato del grupo) y `amount_cents`; el handoff del dueño no publica un conteo de platos por grupo. | «Qué comes · por ingrediente» (2d). El diseño de Mati dice «Pasta · 5 platos» en cada fila y «15 platos pedidos» al centro del anillo. | **No se dice «N platos» por grupo.** La fila dice sus visitas y su monto; el anillo reparte el dinero sobre `total_cents` y lleva al centro el total («$2,165.00 · de consumo»). El subtítulo «Los mismos 7 platos…» usa `distinct_dishes` de «Platos» del mismo período, que sí es un dato del dueño. En «Otros» con una clave desconocida sumada, las visitas no se muestran: dos grupos no se suman sin contar dos veces una visita. | **Pedido aditivo posible al dueño (App Backend), si Mati quiere el «N platos» del diseño.** Anotado 2026-09-19 desde AF-38 (n168). |
