...
| **G-25** | ✅ **CERRADO owner-first.** App Backend v2.71.0 (`064c51a`, inventario completo publicado por `5b1e1f2`) crea un recibo UUID opaco para **cada** POST y proyecta outgoing exclusivamente como `{id, requested_at}` (`contract-mirror/routes/friends.js`); la migración puebla salientes históricas. ⌁ **Original (2026-08-04, preservado):** **`GET /api/friends/requests?direction=outgoing` delata si una cuenta existe, y con quién.** `POST /friends` estaba construido para NO decirlo, pero el backend sólo insertaba fila cuando el destino existía y el GET saliente proyectaba `payme_id`, nombre y apellido. Presencia/ausencia + PII reabrían el oráculo. | Pantalla de amigos, sección "Enviadas". | ✅ **Consumido fail-closed y dual-compatible en frontend v0.147.0.** `contractResponses.ts` acepta POST nuevo/viejo exactos; incoming conserva persona; outgoing nuevo o legacy se proyecta a recibo sin identidad antes de entrar a React. `SocialScreen` cancela por receipt id y sólo retira tras el 200 exacto; error conserva/reconcilia. El mock crea/persiste un recibo por intento sin variar cardinalidad. Tests: `friendRequestContracts.test.ts`, `friendRequestCancellation.test.ts`, `friendRequestsView.test.ts`, `mockFriends.test.ts` y `e2e/g25-recibos-opacos.spec.ts`. ⌁ **Mitigación histórica preservada:** desde 2026-08-04 la vista ya descartaba identidad y evitaba recargar inmediatamente; no cerraba la señal de cardinalidad por sí sola. | ✅ **Pedido satisfecho por el owner y adoptado por el consumidor.** La publicación coordinada permanece Frontend compatible primero y Backend v2.71.0 después; retirar el DTO legacy requiere evidencia posterior de que ya no puede aparecer. ⌁ **Pedido histórico preservado:** crear siempre una salida y proyectarla mediante identificador opaco sin revelar existencia/identidad. |
