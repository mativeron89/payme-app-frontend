...
| --- | --- | --- | --- | --- |
| **G-17** | No existe una transición contractual con epoch/acuse compartido para liberar un tombstone idempotente sin que una pestaña tardía reviva su intención previa. | Nueva mesa, pagos, cargas y transferencias multi-tab. | El front retiene el tombstone y falla cerrado: ningún caller viejo recibe una key nueva. | **P0 · ORDEN 2:** App Backend debe ratificar reconciliación/epoch o acuse server-side antes de habilitar liberación local. |
| **G-18** | `GET /topup/:id` no expone método/attempt id para correlacionar completamente una carga tras reload. | Reconciliación de topup con 3DS. | El front valida id y monto exactos; sin payload/método recuperable no reanuda ni afirma éxito. | **ORDEN 2:** App Backend debe publicar una referencia de intento/método segura si desea reanudación completa. |
| **G-19** | Las versiones publicadas guardaban `payme_idem_*` y `payme_pending_*` en `sessionStorage` sin actor, familia, payload verificable ni referencia contractual de reconciliación. | Upgrade con un hold, pago, carga o transferencia que quedó ambiguo. | El front deja el residuo intacto y crea una cuarentena durable y opaca para el área afectada; no emite una nueva mutación ni atribuye la intención a la sesión actual. | **P0 · ORDEN 2:** App Backend debe ofrecer una reconciliación autenticada que permita acreditar o terminar el intento legacy antes de liberar esa área. |
| **G-20** | El replay de `POST /mesas/:code/pay` selecciona `id`, `status`, `stripe_client_secret` y `gross_amount_cents`, pero omite `tip_amount_cents` y `payment_type` (`contract-mirror/routes/mesas.js:1055-1059`). | Reintento de un pago cuya respuesta se perdió, especialmente con propina. | El front acepta el POST fresco sin `payment_type` porque el request fija el riel, pero rechaza/freezea el replay sin propina: no puede atribuir el total a la propina exacta solicitada. | **P0 · ORDEN 2:** App Backend debe incluir `tip_amount_cents` (y preferentemente `payment_type`) en el SELECT de replay o publicar una referencia contractual equivalente. |
| **G-21** | El replay de `POST /transfers` devuelve la fila de `transfers` sin destinatario verificable (`contract-mirror/routes/transfers.js:22-30,61-65`). | Reintento de transferencia tras respuesta perdida. | El front valida el POST fresco contra importe/destinatario; el replay sin `to.payme_id` queda ambiguo, no navega ni libera/rota el journal. | **P0 · ORDEN 2:** App Backend debe incluir un identificador de destinatario estable y autorizado en el replay para permitir binding seguro. |
| **G-16** | No hay capacidad contractual ni gate IFPE ratificado para habilitar saldo, cargas, transferencias o garantía wallet en un release real. | Wallet/topup/transfer y garantía/pago wallet. | El build real los bloquea antes de red; sólo el mock explícito conserva esos flujos y lleva la tira de demo. | **NO-GO piloto/release real** hasta ratificación de Mati y contrato/capability verificable del App Backend. Anotado en auditoría 2026-08-02. |
