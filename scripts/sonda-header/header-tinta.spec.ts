...
test('sonda · tinta del header', async ({ page, browserName }) => {
  ...
  const data = await page.evaluate(() => {
    ...
    const ub = baselineOf(user), lb = baselineOf(logo); const ui = ink(user, user.innerText), liCap = ink(logo, 'PM');
    const ubox = user.getBoundingClientRect(), lbox = logo.getBoundingClientRect();
    return { squareCy: (rb.y + rb.bottom) / 2, user: { box: [ubox.y, ubox.bottom], baseline: ub, inkCy: (ub - ui.asc + ub + ui.desc) / 2, fontAsc: ui.fAsc, fontDesc: ui.fDesc, transform: getComputedStyle(user).transform, lh: getComputedStyle(user).lineHeight }, logo: { box: [lbox.y, lbox.bottom], baseline: lb, capCy: lb - liCap.asc / 2, fontAsc: liCap.fAsc, fontDesc: liCap.fDesc }, ua: navigator.userAgent.slice(0, 80) };
  });
