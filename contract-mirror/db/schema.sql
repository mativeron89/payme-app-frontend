...
-- ─── EXTERNAL IDENTITY + PAYME RECOVERY · v2.72.0 ─────────
...
CREATE TABLE IF NOT EXISTS auth_recovery_deliveries (
  ...
  delivered_at TIMESTAMPTZ, superseded_at TIMESTAMPTZ, dead_at TIMESTAMPTZ,
  UNIQUE (user_id,generation),
  CONSTRAINT chk_auth_recovery_delivery_state CHECK (
    (state='queued' AND user_id IS NOT NULL AND lease_token IS NULL AND lease_expires_at IS NULL AND delivered_at IS NULL AND superseded_at IS NULL AND dead_at IS NULL)
    OR (state='leased' AND user_id IS NOT NULL AND lease_token IS NOT NULL AND lease_expires_at IS NOT NULL AND delivered_at IS NULL AND superseded_at IS NULL AND dead_at IS NULL)
    OR (state='delivered' AND user_id IS NULL AND lease_token IS NULL AND lease_expires_at IS NULL AND delivered_at IS NOT NULL AND superseded_at IS NULL AND dead_at IS NULL)
    OR (state='superseded' AND user_id IS NULL AND lease_token IS NULL AND lease_expires_at IS NULL AND delivered_at IS NULL AND superseded_at IS NOT NULL AND dead_at IS NULL)
    OR (state='dead' AND user_id IS NULL AND lease_token IS NULL AND lease_expires_at IS NULL AND delivered_at IS NULL AND superseded_at IS NULL AND dead_at IS NOT NULL)
  )
);
CREATE UNIQUE INDEX IF NOT EXISTS uq_auth_recovery_delivery_active_user ON auth_recovery_deliveries(user_id) WHERE state IN ('queued','leased');
CREATE INDEX IF NOT EXISTS idx_auth_recovery_delivery_runnable ON auth_recovery_deliveries(next_attempt_at,created_at) WHERE state='queued';
CREATE INDEX IF NOT EXISTS idx_auth_recovery_delivery_lease ON auth_recovery_deliveries(lease_expires_at) WHERE state='leased';
CREATE INDEX IF NOT EXISTS idx_auth_recovery_delivery_terminal_cleanup ON auth_recovery_deliveries(updated_at) WHERE state IN ('delivered','superseded','dead');
...
CREATE TABLE IF NOT EXISTS auth_recovery_tokens (
  ...
  delivery_id UUID REFERENCES auth_recovery_deliveries(id) ON DELETE SET NULL,
  delivery_attempt INTEGER CHECK (delivery_attempt BETWEEN 1 AND 5),
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_auth_recovery_token_delivery_attempt
  ON auth_recovery_tokens(delivery_id,delivery_attempt) WHERE delivery_id IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS external_auth_intents (
  ...
  status VARCHAR(16) NOT NULL DEFAULT 'issued' CHECK (status IN ('issued','consumed')),
...
CREATE TABLE IF NOT EXISTS external_identity_subject_controls (
  ...
  status VARCHAR(32) NOT NULL CHECK (status IN ('remote_revoked','deletion_suppressed')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
BEGIN
  ...
  IF current_user_row.password_hash IS NULL
     AND NOT EXISTS (SELECT 1 FROM external_identity_bindings
    ...
    RAISE EXCEPTION USING ERRCODE='23514',
      CONSTRAINT='user_auth_method_required', MESSAGE='user_auth_method_required';
  END IF;
END;
$$;
...
BEGIN
  IF ROW(NEW.user_id,NEW.provider,NEW.subject_namespace,NEW.subject)
     IS DISTINCT FROM ROW(OLD.user_id,OLD.provider,OLD.subject_namespace,OLD.subject) THEN
...
CREATE TRIGGER trg_external_binding_immutable BEFORE UPDATE ON external_identity_bindings
...
CREATE CONSTRAINT TRIGGER trg_user_auth_method_users
  ...
  DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION payme_user_auth_method_trigger();
