...
test('D-FF-1 · fragmento→custodia→aviso→alta y limpieza', async ({ page }) => {
  ...
  // U05 · el mock presenta la versión aprobada 2.5.5 de esta entrega. El pin es
  // explícito: el cuerpo visible sigue diciendo DEMOSTRACIÓN y no acredita un
  // aviso legal productivo ni permite aceptar cualquier versión.
  await expect(page.getByText('Versión 2.5.5 ·', { exact: false })).toBeVisible();
