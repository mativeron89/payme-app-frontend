...
async function acreditar(
  ...
  // Revisiones de Mati del 22/09 en su iPhone: 1px (v0.187.2) → 3px (v0.188.1)
  // → iteración 3 (AF-HEADER-ITER3, v0.188.2): igualar LÍNEAS BASE con
  // «PayMe», que es lo que su ojo compara en la foto. `--hdr-user-nudge:
  // 5.5px`, medido en Chromium y WebKit (comentario de `.hdr-user`). Este
  // censo corre en Chromium, donde la tinta del nombre queda 5.01 px por
  // debajo del cuadrado y 5.32 por debajo de las mayúsculas (en WebKit 4.77
  // y 4.40, con las líneas base a 0.08 px). Se acota en Chromium a
  // [4.75, 5.25] respecto del cuadrado: exactamente esta constante.
  const bajada = tinta!.userInkCy - tinta!.squareCy;
  expect(bajada).toBeGreaterThanOrEqual(4.75);
  expect(bajada).toBeLessThanOrEqual(5.25);
  expect(Math.abs(tinta!.userInkCy - tinta!.logoCapCy)).toBeLessThanOrEqual(5.5);
